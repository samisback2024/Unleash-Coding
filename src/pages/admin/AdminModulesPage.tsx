...
function buildColumns(onEdit: (r: Record<string, any>) => void, onDel: (r: Record<string, any>) => void) {
  return [
    ...
    { key: "level", label: "Level", render: (r: Record<string, any>) => (
      <span className={`px-2 py-0.5 rounded text-xs font-medium ${
        r.level === "beginner" ? "bg-green-500/20 text-green-400" :
        r.level === "intermediate" ? "bg-yellow-500/20 text-yellow-400" :
...
export default function AdminModulesPage() {
  ...
  async function load() {
    setLoading(true);
    const [mods, ps] = await Promise.all([getAllModules(), getAllLearningPaths()]);
    setItems(mods);
    setPaths(ps);
    setLoading(false);
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const payload = { ...form, order_index: parseInt(form.order_index, 10) || 0 };
    const result = editingId ? await updateModule(editingId, payload) : await createModule(payload);
    setSaving(false);
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-[#64748b] mt-1">Manage curriculum modules within learning paths</p>
      </div>
      ...
      >
        <div>
          ...
          <input className={fc()} value={form.title} onChange={field("title")} placeholder="Introduction to Python" />
        </div>
        <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <select className={fc()} value={form.level} onChange={field("level")}>
              {LEVELS.map((l) => <option key={l}>{l}</option>)}
            </select>
          </div>
          <div>
            <label className={lc()}>Duration</label>
            <input className={fc()} value={form.duration} onChange={field("duration")} placeholder="2 hours" />
          </div>
          <div>
      ...
      >
        <p className="text-sm text-[#94a3b8]">
          Delete <span className="font-semibold text-[#f1f5f9]">{deleting?.title}</span>?
          All lessons within this module will also be deleted. This cannot be undone.
        </p>
      </AdminModal>
    </div>
  );
