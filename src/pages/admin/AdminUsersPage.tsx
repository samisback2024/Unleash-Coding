...
export default function AdminUsersPage() {
  ...
  async function toggleRole(u: AdminUser) {
    ...
    showToast(`${u.username || u.fullName} is now ${newRole === "admin" ? "an admin" : "a regular user"}.`);
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-[#64748b] mt-1">Manage user roles and view learner stats</p>
        </div>
          ...
          {(["all", "user", "admin"] as const).map((r) => (
            <button
              ...
              className={`px-3 py-1 rounded text-xs font-medium transition-colors capitalize ${
                roleFilter === r ? "bg-[#6c63ff] text-white" : "text-[#94a3b8] hover:text-[#f1f5f9]"
              }`}
            >
              {r === "all" ? "All Roles" : r}
            </button>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#2a2d3e]">
                {["User", "Username", "Role", "XP", "Level", "Streak", "Joined", ""].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="px-4 py-12 text-center text-sm text-[#64748b]">
                  No users found.
                </td></tr>
              ) : filtered.map((u) => (
                <tr key={u.id} className="border-b border-[#2a2d3e] last:border-0 hover:bg-[#252840] transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-[#f1f5f9] truncate max-w-[120px]">{u.fullName || u.username}</span>
                      {u.id === currentUser?.id && (
                        <span className="text-[10px] text-[#64748b]">(you)</span>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-sm text-[#94a3b8]">{u.username}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-sm text-[#94a3b8]">{u.xp.toLocaleString()}</td>
                  <td className="px-4 py-3 text-sm text-[#94a3b8]">{u.level}</td>
                  <td className="px-4 py-3 text-sm text-[#94a3b8]">{u.streak} 🔥</td>
                  <td className="px-4 py-3 text-xs text-[#64748b]">{timeAgo(u.createdAt)}</td>
                  <td className="px-4 py-3">
                    <button
