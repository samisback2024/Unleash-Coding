...
export default function AdminProjectsPage() {
  ...
  async function loadProjects() {
    setProjLoading(true);
    const [ps, paths_] = await Promise.all([
      getAllProjects(),
      getAllLearningPaths(),
    ]);
    setProjects(ps);
  ...
  function openEdit(row: Record<string, any>) {
    ...
    setForm({
      ...
      requirements: Array.isArray(row.requirements)
        ...
        : (row.requirements ?? ""),
      skills_covered: Array.isArray(row.skills_covered)
        ? row.skills_covered.join(", ")
        : (row.skills_covered ?? ""),
  ...
  async function toggleFeature(sub: AdminShowcaseSubmission) {
    const result = sub.isFeatured
      ? await unfeatureProject(sub.id)
      : await featureProject(sub.id);
    if (result.error) {
      showToast(result.error, "error");
      return;
    }
  ...
  async function togglePublic(sub: AdminShowcaseSubmission) {
    ...
    showToast(
      sub.isPublic ? "Hidden from community." : "Visible in community.",
    );
    loadShowcase();
  }

  async function handleStatusChange(
    sub: AdminShowcaseSubmission,
    status: string,
  ) {
  ...
  const projectColumns = [
    ...
    },
    {
      key: "xp_reward",
      label: "XP",
      render: (r: Record<string, any>) => `${r.xp_reward ?? 0} XP`,
    },
    {
      key: "is_capstone",
      label: "Capstone",
      render: (r: Record<string, any>) => (r.is_capstone ? "✓" : ""),
    },
    {
      key: "actions",
      label: "",
    ...
    key: string;
    label: string;
    render?: (r: Record<string, any>) => React.ReactNode;
    width?: string;
  }[];

  const showcaseFiltered = showcase.filter(
    (s) =>
    ...
    <div className="p-6 space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-[#64748b] mt-1">
          Manage portfolio projects and community showcase submissions
        </p>
      </div>
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            className={`p-1.5 rounded transition-colors ${s.isPublic ? "text-green-400 hover:bg-green-400/10" : "text-[#64748b] hover:text-[#94a3b8] hover:bg-[#252840]"}`}
                          >
                            {s.isPublic ? (
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            className={`p-1.5 rounded transition-colors ${s.isFeatured ? "text-yellow-400 hover:bg-yellow-400/10" : "text-[#64748b] hover:text-yellow-400 hover:bg-yellow-400/10"}`}
                          >
                            <Star
                              className={`w-4 h-4 ${s.isFeatured ? "fill-current" : ""}`}
                            />
                          </button>
                        </td>
      ...
      <AdminModal
        title={
          editingId ? "Edit Portfolio Project" : "Create Portfolio Project"
        }
        isOpen={isModalOpen}
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              className={fc()}
              value={form.portfolio_level}
              onChange={field("portfolio_level")}
            >
              {PORTFOLIO_LEVELS.map((pl) => (
          ...
          <div>
            ...
            <input
              ...
              onChange={field("estimated_hours")}
            />
          </div>
          <div>
          ...
          <div className="col-span-2">
            ...
            <textarea
              ...
              placeholder="Implement CRUD operations&#10;Use local storage&#10;Make it responsive"
            />
          </div>
          <div className="col-span-2">
            ...
            <input
              ...
              onChange={field("skills_covered")}
              placeholder="Python, OOP, File I/O"
            />
          </div>
          <div>
          ...
          <div className="flex items-center gap-2 mt-5">
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, is_capstone: e.target.checked }))
              }
              className="w-4 h-4 accent-[#6c63ff]"
            />
            <label htmlFor="is_capstone" className="text-sm text-[#94a3b8]">
              Capstone project
            </label>
          </div>
        </div>
